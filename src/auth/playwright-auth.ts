...
/**
 * Launches a browser in headed mode, navigates to the ChatOps base URL, and
...
export async function runInteractiveLogin(options: {
  ...
  if (!isValid) {
    ...
    throw new Error(
      ...
      "Please run chatops-auth-login again and ensure you reach the ChatOps dashboard before closing the browser."
    );
  }
...
export async function validateCandidateSession(
  ...
  try {
    ...
    // Guard against ChatOps returning HTML login page with 200 OK
