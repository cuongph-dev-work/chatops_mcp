...
export async function loadAndValidateSession(
  ...
  try {
    const res = await axios.get(validateUrl, {
      ...
      // 3xx redirects mean ChatOps is bouncing us to an SSO/login page.
      validateStatus: (status) => status >= 200 && status < 300,
    });

    // Guard against login page returned with 200 OK
    ...
    // Re-throw McpErrors (sessionExpired thrown above) and network errors
    throw err;
  }
}
